...
@section('content')
    <div class="p-6">
                        ...
                        <div class='flex flex-col gap-2 shrink-0 sm:flex-row'>
                            <div class='w-full md:w-72'>
                                <div class='relative h-10 w-full min-w-[200px]'>
                                    ...
                                    <form id="searchForm" method="GET" action="{{ route('admin.manage.manageRoom') }}">
                                        <input id="search" name="keyword"
                                            class='peer h-full w-full rounded-[7px] border border-blue-gray-200 border-t-transparent bg-transparent px-3 py-2.5 !pr-9 font-sans text-sm font-normal text-blue-gray-700 outline outline-0 transition-all placeholder-shown:border placeholder-shown:border-blue-gray-200 placeholder-shown:border-t-blue-gray-200 focus:border-2 focus:border-gray-900 focus:border-t-transparent focus:outline-0 disabled:border-0 disabled:bg-blue-gray-50'
                                            placeholder='{{ __('content.manage-room.search_typeroom') }}'
                                    ...
                                    <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                                        ...
                                        <div class="p-6">
                                            ...
                                            <form method="GET" id="advancedSearchForm"
                                                action="{{ route('admin.manage.manageRoom') }}">
                                                @csrf
                                                <div class="grid grid-cols-2 gap-4">
                                                    <!-- Loại phòng -->
                                                    <div>
                                                        <label for="roomType"
                                                            class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">{{ __('content.manage-room.typeRoom') }}</label>
                                                        <select id="roomType" name="roomType"
                                                            class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                                                            <option value="">{{ __('content.manage-room.all') }}
                                                            </option>
                                                    ...
                                                    <div>
                                                        <label for="maxGuests"
                                                            class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">{{ __('content.manage-room.guest_quantity') }}</label>
                                                        <input type="number" id="maxGuests" name="maxGuests"
                                                            class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                                                            placeholder="{{ __('content.manage-room.number_guest') }}"
                                                            value="{{ old('maxGuests', request('maxGuests')) }}">
                                                    </div>
                                                    <!-- booker -->
                                                    <div>
                                                        <label for="bookerName"
                                                            class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">{{ __('content.manage-room.user') }}</label>
                                                        <input type="text" id="bookerName" name="bookerName"
                                                            class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                                                            placeholder="{{ __('content.manage-booking.user_name_booking') }}"
                                                            value="{{ old('bookerName', request('bookerName')) }}">
                                                    </div>
                                                    <!-- phone number -->
                                                    <div>
                                                        <label for="phoneNumber"
                                                            class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">{{ __('content.manage-room.phone_number') }}</label>
                                                        <input type="text" id="phoneNumber" name="phoneNumber"
                                                            class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                                                            placeholder="{{ __('content.manage-booking.phone_booking') }}"
                                                            value="{{ old('phoneNumber', request('phoneNumber')) }}">
                                                    </div>
                                                    <!-- status -->
                                                    <div>
                                                        <label for="status"
                                                            class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">{{ __('content.manage-room.status') }}</label>
                                                        <select id="status" name="status"
                                                            class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                                                            <option value=""
                                                                {{ old('status') == '' ? 'selected' : '' }}>
                                                                {{ __('content.manage-room.all') }}</option>
                                                            <option value="available"
                                                                {{ old('status') == 'available' ? 'selected' : '' }}>
                                                                {{ __('content.manage-room.Available') }}</option>
                                                            <option value="booked"
                                                                {{ old('status') == 'booked' ? 'selected' : '' }}>
                                                                {{ __('content.manage-room.Booked') }}</option>
                                                        </select>
                                                    </div>
                                                </div>
                                            </form>
                                        ...
                                        <!-- Modal Footer -->
                                        <div
                                            class="flex justify-end p-4 border-t border-gray-200 rounded-b dark:border-gray-600">
                                            <button type="button"
                    ...
                    <table class="items-center w-full bg-transparent border-collapse">
                        <thead>
                            <tr>
                                ...
                                <th
                                    class="px-4 bg-gray-100 dark:bg-gray-600 text-gray-500 dark:text-gray-100 align-middle border border-solid border-gray-200 dark:border-gray-500 py-3 text-xs uppercase border-l-0 border-r-0 whitespace-nowrap font-semibold text-left">
                                    {{ __('content.manage-room.guest_quantity') }}</th>
                                <th
                                    class="px-4 bg-gray-100 dark:bg-gray-600 text-gray-500 dark:text-gray-100 align-middle border border-solid border-gray-200 dark:border-gray-500 py-3 text-xs uppercase border-l-0 border-r-0 whitespace-nowrap font-semibold text-left">
                                    {{ __('content.manage-room.user') }}</th>
                                <th
                                    class="px-4 bg-gray-100 dark:bg-gray-600 text-gray-500 dark:text-gray-100 align-middle border border-solid border-gray-200 dark:border-gray-500 py-3 text-xs uppercase border-l-0 border-r-0 whitespace-nowrap font-semibold text-left">
                                    {{ __('content.manage-room.phone_number') }}</th>
                                <th
                                    class="px-4 bg-gray-100 dark:bg-gray-600 text-gray-500 dark:text-gray-100 align-middle border border-solid border-gray-200 dark:border-gray-500 py-3 text-xs uppercase border-l-0 border-r-0 whitespace-nowrap font-semibold text-left">
                                    {{ __('content.manage-room.time') }}</th>
                                <th
                                    class="px-4 bg-gray-100 dark:bg-gray-600 text-gray-500 dark:text-gray-100 align-middle border border-solid border-gray-200 dark:border-gray-500 py-3 text-xs uppercase border-l-0 border-r-0 whitespace-nowrap font-semibold text-left">
                                    {{ __('content.manage-room.price') }}</th>
                                <th
                                    ...
                                    {{ __('content.manage-room.status') }}</th>
                                <th
                                    class="px-4 bg-gray-100 dark:bg-gray-600 text-gray-500 dark:text-gray-100 align-middle border border-solid border-gray-200 dark:border-gray-500 py-3 text-xs uppercase border-l-0 border-r-0 whitespace-nowrap font-semibold text-left min-w-140-px">
                        ...
                        <tbody id='manage-room'>
                            ...
                            @else
                                @include('admin.manage-room.table-room')
                            @endif
                        </tbody>
                    </table>
                </div>
        ...
        <div class="w-full flex justify-between items-center p-0">{{ $roomList->links('admin.layouts.pagination') }}</div>
    </div>
    {{-- @include('admin.manage-room.modal-room') --}}
@endsection
@push('scripts')
    <script>
