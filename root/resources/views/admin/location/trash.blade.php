                    ...
                    <div class='flex items-center justify-between gap-8 mb-8'>
                        <div>
                            <h5
                                ...
                                {{ __('content.location.trash_list') }}
                            </h5>
                        </div>
                        <div class='flex flex-col gap-2 shrink-0 sm:flex-row'>
                            <a href="{{ route('admin.locations.index') }}" 
                                class='bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded'>
                                {{ __('content.location.back') }}
                            </a>
                        </div>
                    </div>
                </div>
                        ...
                        <thead>
                            <tr>
                                <th
                                    ...
                                    {{ __('content.location.id') }}</th>
                                <th
                                    class="px-4 bg-gray-100 align-middle border border-solid border-gray-200 py-3 text-xs uppercase border-l-0 border-r-0 whitespace-nowrap font-semibold text-left">
                                    {{ __('content.location.name') }}</th>
                                <th
                                    class="px-4 bg-gray-100 align-middle border border-solid border-gray-200 py-3 text-xs uppercase border-l-0 border-r-0 whitespace-nowrap font-semibold text-left">
                                    {{ __('content.location.category_image') }}</th>
                                <th
                                    class="px-4 bg-gray-100 align-middle border border-solid border-gray-200 py-3 text-xs uppercase border-l-0 border-r-0 whitespace-nowrap font-semibold text-left">
                                    {{ __('content.location.description') }}</th>
                                <th
                                    class="px-4 bg-gray-100 align-middle border border-solid border-gray-200 py-3 text-xs uppercase border-l-0 border-r-0 whitespace-nowrap font-semibold text-left">
                                    {{ __('content.location.deleted_at') }}</th>
                                <th
                                ...
                                <tr class="text-gray-700">
                                    ...
                                    <td
                                        ...
                                        @if ($location->images->isNotEmpty())
                                            ...
                                            @endphp
                                            <button class="open-modal-btn" data-id="{{ $location->id }}">
                                                <img src="{{ asset('storage/' . $image->image) }}" alt=""
                                                    class="h-24 w-24 object-cover rounded-md shadow-sm border border-gray-300">
                                            </button>
                                        @else
                                            <p>{{ __('content.location.no_image') }}</p>
                                        @endif
                                    </td>
                                    <td
                                        class="border-t-0 px-4 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4">
                                        {{ $location->description ? $location->description : __('content.location.no_description') }}
                                    </td>
                                    <td
                                        class="border-t-0 px-4 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4">
                                        {{ $location->deleted_at->format('d/m/Y H:i:s') }}
                                    </td>
                                    <td
                                        class="border-t-0 px-4 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4">
                                        <button
                                        ...
                                        <button
                                            ...
                                            type='button' title='{{ __('content.location.delete_permanent') }}'
                                            data-id='{{ $location->id }}'>
            ...
            $(document).on('click', '.restore-button', function() {
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        // Gửi yêu cầu AJAX
                        $.ajax({
                            url: `/admin/locations/restore/${id}`,
                            type: 'POST',
                            data: {
            ...
            $(document).on('click', '.delete-permanent-button', function() {
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        ...
                        $.ajax({
                            url: `/admin/locations/forceDelete/${id}`,
                            type: 'GET',
                            data: {
                                _token: $('meta[name="csrf-token"]').attr('content')
                            },
