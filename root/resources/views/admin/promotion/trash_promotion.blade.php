                    ...
                    <div class='flex items-center justify-between gap-8 mb-8'>
                        <div>
                            <h5
                                ...
                                {{ __('content.promotion.restore_coupon_code') }}
                            </h5>
                        </div>
                        <div class='flex flex-col gap-2 shrink-0 sm:flex-row'>
                            <a href="{{ route('admin.promotion.index') }}"
                                class='flex select-none items-center gap-3 rounded-lg bg-green-600 py-4 px-6 text-center align-middle font-sans text-xs font-bold uppercase text-white shadow-md shadow-green-600/10 transition-all hover:bg-green-700 hover:shadow-lg hover:shadow-green-600/20 focus:opacity-[0.85] focus:shadow-none active:opacity-[0.85] active:shadow-none disabled:pointer-events-none disabled:opacity-50 disabled:shadow-none'
                                type='button'>
                                {{ __('content.promotion.Back_to_list_page') }}
                            </a>
                        </div>
                    </div>
                </div>
                ...
                <div class="block w-full overflow-x-auto">
                    <table class="items-center w-full bg-transparent border-collapse">
                        ...
                        <tbody id="promotion-list">
                            @foreach ($listDeletedPromotions as $promotion)
                                <tr class="text-gray-700">
                                    ...
                                    <td
                                        ...
                                        <button
                                            ...
                                            type='button' title='{{ __('content.promotion.restore') }}' data-id='{{ $promotion->id }}'>
                                        ...
                                        <button
                                            ...
                                            type='button' title='{{ __('content.promotion.delete_permanent') }}'
                    ...
                    <div class="w-full flex justify-between items-center p-0">
                        {{ $listDeletedPromotions->links('admin.layouts.pagination') }}</div>
                    </div>
            ...
            $(document).on('click', '.restore-button', function() {
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        // Gửi yêu cầu AJAX
                        $.ajax({
                            url: `/admin/promotion/restore/${id}`,
                            type: 'POST',
                            data: {
                            ...
                            error: function() {
                                Swal.fire({
                                    icon: 'error',
                                    title: 'Lỗi!',
                                    text: 'Không thể khôi phục mã giảm giá này'
                                });
                            }
                        });
                    }
        ...
        $(document).ready(function() {
            $(document).on('click', '.delete-permanent-button', function() {
                ...
                Swal.fire({
                    ...
                    text: "Mã giảm giá này sẽ bị xóa vĩnh viễn và không thể khôi phục!",
                    icon: 'warning',
                    showCancelButton: true,
                    ...
                    if (result.isConfirmed) {
                        ...
                        $.ajax({
                            url: `/admin/promotion/forceDelete/${id}`,
                            type: 'GET',
                            data: {
                                _token: $('meta[name="csrf-token"]').attr('content')
                            },
