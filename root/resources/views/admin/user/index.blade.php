...
@section('content')
    <div class="p-6">
                        ...
                        <div class='flex flex-col gap-2 shrink-0 sm:flex-row'>
                            <div class='w-full md:w-72'>
                                <div class='relative h-10 w-full min-w-[200px]'>
                                    ...
                                    <form id="searchForm" method="GET" action="{{ route('admin.user.index') }}">
                                        <input id="search" name="keyword"
                                            class='peer h-full w-full rounded-[7px] border border-blue-gray-200 border-t-transparent bg-transparent px-3 py-2.5 !pr-9 font-sans text-sm font-normal text-blue-gray-700 outline outline-0 transition-all placeholder-shown:border placeholder-shown:border-blue-gray-200 placeholder-shown:border-t-blue-gray-200 focus:border-2 focus:border-gray-900 focus:border-t-transparent focus:outline-0 disabled:border-0 disabled:bg-blue-gray-50'
                                            placeholder='{{ __('content.user.search_user') }}' value="{{ old('keyword', request('keyword')) }}"/>
                                        <label
                            ...
                            <div id="advancedSearchModal" tabindex="-1" aria-hidden="true"
                                ...
                                <div class="relative w-full max-w-lg max-h-full">
                                    <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                                        ...
                                        <div class="p-6">
                                            <!-- Search Form -->
                                            <form method="GET" id="advancedSearchForm" action="{{ route('admin.user.index')}}" >
                                                @csrf
                                                <div class="grid grid-cols-2 gap-4">
                                                    <!-- maxGuests -->
                                                    <div>
                                                        <label for="name" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">{{ __('content.user.name') }}</label>
                                                        <input type="text" id="name" name="name"
                                                            class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                                                            placeholder="{{ __('content.user.search_user') }}" value="{{ old('name', request('name')) }}" >
                                                    </div>
                                                    <!-- booker -->
                                                    <div>
                                                        <label for="email" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">{{ __('content.user.email') }}</label>
                                                        <input type="text" id="email" name="email"
                                                            class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                                                            placeholder="{{ __('content.user.email_search') }}" value="{{ old('email', request('email')) }}">
                                                    </div>
                                                    <!-- phone number -->
                                                    <div>
                                                        <label for="phoneNumber" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">{{ __('content.user.phone_number') }}</label>
                                                    ...
                                                    <!-- status -->
                                                    <div>
                                                        <label for="status" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">{{ __('content.user.status') }}</label>
                                                        <select id="status" name="status"
                                                            class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                                                            <option value="" {{ old('status') == '' ? 'selected' : '' }}>{{ __('content.manage-room.all') }}</option>
                                                            <option value="1" {{ old('status') == '1' ? 'selected' : '' }}>{{ __('content.user.active') }}</option>
                                                            <option value="0" {{ old('status') == '0' ? 'selected' : '' }}>{{ __('content.user.ban') }}</option>
                                                        </select>
                                                    </div>
                                                </div>
                                            </form>
                        ...
                        <thead>
                            <tr>
                                <th
                                    ...
                                    {{ __('content.user.avatar') }}</th>
                                <th
                                    class="px-4 bg-gray-100 text-xs uppercase border-l-0 border-r-0 whitespace-nowrap font-semibold text-left">
                                    {{ __('content.user.name') }}</th>
                                <th
                                    class="px-4 bg-gray-100 text-xs uppercase border-l-0 border-r-0 whitespace-nowrap font-semibold text-left">
                                    {{ __('content.user.email') }}</th>
                                <th
                                    class="px-4 bg-gray-100 text-xs uppercase border-l-0 border-r-0 whitespace-nowrap font-semibold text-left">
                                    {{ __('content.user.phone_number') }}</th>
                                <th
                                    class="px-4 bg-gray-100 text-xs uppercase border-l-0 border-r-0 whitespace-nowrap font-semibold text-left">
                                    {{ __('content.user.status') }}</th>
                                <th
                                    class="px-4 bg-gray-100 text-xs uppercase border-l-0 border-r-0 whitespace-nowrap font-semibold text-left">
                                    {{ __('content.user.action') }}</th>
                            </tr>
                        </thead>
        ...
        <div class="w-full flex justify-between items-center p-0">{{ $users->links('admin.layouts.pagination') }}</div>
    </div>
@endsection
@push('scripts')
<script>
