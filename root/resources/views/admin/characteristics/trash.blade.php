                    ...
                    <div class='flex items-center justify-between gap-8 mb-8'>
                        <div>
                            <h5
                                ...
                                {{ __('content.characteristic.trash_list') }}
                            </h5>
                        </div>
                        <div class='flex flex-col gap-2 shrink-0 sm:flex-row'>
                            <a href="{{ route('admin.characteristics.index') }}" 
                                class='bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded'>
                                {{ __('content.characteristic.back') }}
                            </a>
                        </div>
                    </div>
                </div>
                        ...
                        <thead>
                            <tr>
                                <th
                                    ...
                                    {{ __('content.characteristic.id') }}</th>
                                <th
                                    class="px-4 bg-gray-100 align-middle border border-solid border-gray-200 py-3 text-xs uppercase border-l-0 border-r-0 whitespace-nowrap font-semibold text-left">
                                ...
                                <th
                                    class="px-4 bg-gray-100 align-middle border border-solid border-gray-200 py-3 text-xs uppercase border-l-0 border-r-0 whitespace-nowrap font-semibold text-left">
                                    {{ __('content.characteristic.deleted_at') }}</th>
                                <th
                        ...
                        <tbody id="characteristics-list">
                            @foreach ($characteristics as $characteristic)
                                <tr class="text-gray-700">
                                    ...
                                    <td
                                        class="border-t-0 px-4 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4">
                                        {{ $characteristic->deleted_at->format('d/m/Y H:i:s') }}
                                    </td>
                                    <td
                                        class="border-t-0 px-4 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4">
                                        <button
                                        ...
                                        <button
                                            ...
                                            type='button' title='{{ __('content.characteristics.delete_permanent') }}'
                                            data-id='{{ $characteristic->id }}'>
            ...
            $(document).on('click', '.restore-button', function() {
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        // Gửi yêu cầu AJAX
                        $.ajax({
                            url: `/admin/characteristics/restore/${id}`,
                            type: 'POST',
                            data: {
            ...
            $(document).on('click', '.delete-permanent-button', function() {
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        ...
                        $.ajax({
                            url: `/admin/characteristics/forceDelete/${id}`,
                            type: 'GET',
                            data: {
                                _token: $('meta[name="csrf-token"]').attr('content')
                            },
