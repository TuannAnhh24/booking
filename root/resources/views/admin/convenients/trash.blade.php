                    ...
                    <table class="items-center w-full bg-transparent border-collapse">
                        <thead>
                            <tr>
                                ...
                                <th
                                    class="px-4 bg-gray-100 align-middle border border-solid border-gray-200 py-3 text-xs uppercase border-l-0 border-r-0 whitespace-nowrap font-semibold text-left">
                                    {{ __('content.convenient.deleted_at') }}</th>
                                <th
                        ...
                        <tbody id="convenient-list">
                            @foreach ($convenients as $convenient)
                                <tr class="text-gray-700">
                                    ...
                                    <td
                                        class="border-t-0 px-4 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4">
                                        {{ $convenient->deleted_at->format('d/m/Y H:i:s') }}
                                    </td>
                                    <td
                                        class="border-t-0 px-4 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4">
                                        <button
                                        ...
                                        <button
                                            ...
                                            type='button' title='{{ __('content.convenient.delete_permanent') }}'
                                            data-id='{{ $convenient->id }}'>
            ...
            $(document).on('click', '.restore-button', function() {
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        // Gửi yêu cầu AJAX
                        $.ajax({
                            url: `/admin/convenients/restore/${id}`,
                            type: 'POST',
                            data: {
            ...
            $(document).on('click', '.delete-permanent-button', function() {
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        ...
                        $.ajax({
                            url: `/admin/convenients/forceDelete/${id}`,
                            type: 'GET',
                            data: {
                                _token: $('meta[name="csrf-token"]').attr('content')
                            },
